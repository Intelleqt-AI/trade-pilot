      ...
      >
        {normalised.map((item) => (
          <TabsPrimitive.Trigger
            ...
            value={item.value}
            className={cn(
              'group inline-flex h-full items-center gap-1.5 whitespace-nowrap rounded-md font-semibold text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-primary/25 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-xs',
              size === 'sm' ? 'px-2.5 text-xs' : 'px-3 text-[13px]'
            )}
          >
            {item.label}
            {item.count !== undefined && (
              <span className="rounded bg-muted px-1 font-mono text-[11px] font-semibold tabular-nums text-muted-foreground group-data-[state=active]:bg-primary/10 group-data-[state=active]:text-primary">
                {item.count}
              </span>
            )}
          </TabsPrimitive.Trigger>
