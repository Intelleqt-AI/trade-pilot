...
export default function ElectriciansHero({
  ...
  heading = "Find Local Electricians Near You",
  ...
  return (
    <section aria-labelledby="trade-hero-title" className="relative overflow-hidden bg-[#001F3D] font-sans lg:min-h-[440px]">
      <img src={imageSrc} alt={imageAlt} style={{ objectPosition: imagePosition }} className={`absolute inset-y-0 right-0 hidden h-full w-[calc(50%-20px)] bg-[#001F3D] object-${imageFit} lg:block`} />
      ...
      <div className="relative mx-auto max-w-[1200px] px-4 pb-7 pt-7 lg:px-8 lg:pb-9 lg:pt-10 xl:px-0">
        <div className="flex flex-col lg:w-[calc(50%-60px)] xl:w-[580px]">
          <h1 id="trade-hero-title" className="text-[30px] font-bold leading-[1.15] tracking-[-0.02em] text-white lg:text-[42px] lg:leading-[1.1]">
            <span className="lg:hidden">{heading}</span>
            <span className="hidden lg:inline">{heading}</span>
          </h1>
          <p className="mt-2.5 max-w-[500px] text-base leading-normal text-[#C8D5E2] lg:text-[17px]">{description}</p>
          <div className="mt-[22px] w-full lg:max-w-[460px]">
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} noValidate aria-label={`Compare ${tradeName.toLowerCase()} quotes`} className="flex flex-col">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <label htmlFor={inputId} className="text-[15px] font-semibold text-white">Your postcode</label>
                  <span className="inline-flex h-[30px] items-center gap-1.5 rounded-full border border-[#1DAFA1]/60 bg-[#1DAFA1]/15 pl-2.5 pr-3 text-[13px] text-white"><Zap aria-hidden="true" className="h-3.5 w-3.5 text-[#1DAFA1]" /><span className="text-[#A9BDD0]">Job type</span><span className="font-semibold">{tradeName}</span></span>
                </div>
